...
const Header: React.FC = () => {
    // THE FIX: We now get the profile directly from our upgraded AuthContext
    const { user, profile } = useAuth(); 
    const navigate = useNavigate();
    const [dropdownOpen, setDropdownOpen] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);
    // ... (Search logic can remain the same) ...
    ...
    // This useEffect is for closing the dropdown when clicking outside
    useEffect(() => {
            ...
            <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-dark-secondary border border-dark-tertiary rounded-md shadow-lg py-1">
                        {profile && (
                            // THIS LINK WILL NOW ALWAYS APPEAR WHEN LOGGED IN
